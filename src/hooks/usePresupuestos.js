...
import {
  getPresupuestosMesActual,
  crearPresupuesto, editarPresupuesto,
  desactivarPresupuesto, clonarPresupuestosMesAnterior,
...
export function usePresupuestos() {
  ...
  const cargar = useCallback(async () => {
    try {
      setCargando(true)
      setError(null)
      const data = await getPresupuestosMesActual()
  ...
  // Resumen para el widget del Dashboard
  const resumen = {
    total:     presupuestos.length,
    excedidos: presupuestos.filter(p => p.porcentaje > 100).length,
    alertas:   presupuestos.filter(p =>
                 p.porcentaje >= p.alerta_pct && p.porcentaje <= 100).length,
