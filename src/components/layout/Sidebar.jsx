...
export function Sidebar({ usuario }) {
  ...
  return (
    <aside 
      ...
      className={`hidden md:flex flex-col h-screen fixed z-40 
      ...
      <div className="p-4 flex flex-col h-full">
        ...
        <div className={`flex items-center gap-3 mb-8 ${expandido ? 'px-2' : 'justify-center'}`}>
          ...
          <span className={`text-2xl font-extrabold bg-gradient-to-br from-[var(--mango)] to-[var(--mango-dark)] bg-clip-text text-transparent tracking-tight 
            ...
            ${expandido ? 'w-auto opacity-100' : 'w-0 opacity-0'}`}>
            Manguito
          </span>
        </div>
          ...
          {menu.map(item => {
            ...
            return (
              ...
              >
                ...
                <span className={`transition-all duration-300 overflow-hidden whitespace-nowrap text-sm
                  ${expandido ? 'w-auto opacity-100' : 'w-0 opacity-0'}`}>
                  {item.label}
                </span>
              </Link>
