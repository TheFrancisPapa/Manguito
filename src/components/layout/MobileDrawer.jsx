// src/components/layout/MobileDrawer.jsx — ACTUALIZADO con nuevas secciones
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800">
          ...
          <button onClick={onCerrar}
            className="w-8 h-8 flex items-center justify-center bg-zinc-100 dark:bg-zinc-800
              text-zinc-600 dark:text-zinc-400 rounded-full active:scale-90 transition-transform text-sm">
            ✕
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3 flex flex-col gap-1">
          {Object.entries(itemsPorGrupo).map(([grupo, items]) => (
            <div key={grupo}>
              <p className="text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-600 px-3 py-2 mt-2">
                {GRUPOS[grupo]}
              </p>
              {items.map(item => (
                <Link key={item.path} to={item.path} onClick={onCerrar}
        ...
        <div className="p-4 border-t border-zinc-100 dark:border-zinc-800">
          <button onClick={() => { toggleTheme(); onCerrar() }}
            className="w-full flex items-center justify-center gap-2 p-3.5 rounded-2xl
              bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800
              text-sm font-semibold text-zinc-700 dark:text-zinc-300 active:scale-95 transition-all shadow-sm">
