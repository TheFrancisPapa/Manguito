export function Button({ children, variante = 'primary', className = '', icono, cargando, tamaño, ...props }) {
  const base = "inline-flex items-center justify-center gap-2 rounded-2xl font-semibold transition-all duration-200 active:scale-[0.97] disabled:opacity-60 disabled:pointer-events-none cursor-pointer select-none"
  ...
  const variantes = {
    ...
    secondary: `bg-white dark:bg-[var(--dark-card)] text-zinc-700 dark:text-zinc-200 
      ...
      hover:border-[var(--mango)]/30 shadow-sm`,
    
    danger: "bg-gradient-to-br from-red-500 to-rose-600 text-white shadow-md shadow-red-500/25 hover:shadow-red-500/40 hover:-translate-y-0.5",
