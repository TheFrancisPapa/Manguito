export function EmptyState({ icono = '📭', titulo, descripcion, accion = null }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-1">{titulo}</h3>
      {descripcion && <p className="text-xs text-zinc-400 max-w-xs mb-5">{descripcion}</p>}
      {accion}
    </div>
  )
}
