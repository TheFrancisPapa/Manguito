...
// Gráfico de línea de ingresos vs gastos por día/semana.
...
export function LineaTemporal({ datos = [], moneda = 'ARS', cargando = false }) {
  ...
  if (cargando) {
    return <div className="w-full h-40 rounded-xl bg-zinc-100 dark:bg-zinc-800 animate-pulse" />
  }
  ...
  const linea = (campo, color) => {
    const puntos = datos.map((d, i) => `${px(i)},${py(d[campo])}`).join(' ')
    return (
      <polyline
        points={puntos}
        fill="none"
      ...
      >
        ...
        {datos.map((d, i) => (
          <g key={i} onMouseEnter={() => setHover(i)}>
            ...
            {/* Área invisible para capturar hover fácilmente */}
            <rect
              x={px(i) - xStep / 2} y={PAD.t}
