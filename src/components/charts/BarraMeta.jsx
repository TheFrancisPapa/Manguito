...
// Barra de progreso grande para una meta de ahorro.
// Se diferencia de PresupCard en que es más visual
// y muestra el ícono + falta + fecha prominentes.
...
export function BarraMeta({ meta, moneda = 'ARS', onAportar }) {
  ...
  const diasRestantes = fecha_limite
    ? Math.ceil(
        (new Date(fecha_limite + 'T00:00:00') - new Date()) / 86_400_000
      )
    : null
    ...
    ">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-zinc-400">
            {completa ? '¡Meta alcanzada! 🎉' : `Falta ${fmt(falta)}`}
          </p>
        </div>
        {!completa && onAportar && (
          <button
      ...
      <div className="mt-2 text-right">
        <span
          ...
          style={{ color: completa ? '#10B981' : color }}
        >
          {pct.toFixed(0)}%
        </span>
      </div>
