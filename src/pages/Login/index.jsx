...
export function LoginPage() {
  ...
  const [form, setForm]         = useState({
    nombre: '', email: '', password: '', moneda: 'ARS', usarDemo: true,
  })
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {modo === 'registro' && (
            <>
              ...
              <label className="flex items-start gap-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.usarDemo}
                  onChange={e => setForm(f => ({ ...f, usarDemo: e.target.checked }))}
                  className="mt-0.5 accent-amber-400"
                />
                <span className="text-sm text-zinc-600 dark:text-zinc-400">
                  Cargar datos de ejemplo para explorar la app
                </span>
              </label>
            </>
          )}
          ...
          {/* Error */}
          {error && (
            <p className="text-xs text-red-500 bg-red-50 dark:bg-red-900/20
                          rounded-xl px-3 py-2 border border-red-100
                          dark:border-red-900">
              {error}
            </p>
          )}
          ...
          <p className="text-center text-xs text-zinc-400 mt-5">
            <button
              ...
              onClick={() => {
                ...
                if (email) useAuthContext().recuperarPassword(email)
              }}
            >
