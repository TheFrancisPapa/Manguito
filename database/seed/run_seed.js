...
import { seedCategorias, seedMovimientos, seedPresupuestos, seedMetas } from './categorias'
...
export async function seedDemo(usuarioId) {
  const { data: cats, error } = await supabase
    .from('categorias').select('id, nombre').eq('usuario_id', usuarioId)
  ...
  await supabase.from('usuarios').update({ onboarding_ok: true }).eq('id', usuarioId)
